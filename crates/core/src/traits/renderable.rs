    ...
    fn generate_mesh(&self, render_options: RenderOptions) -> bevy::prelude::Mesh {
        match render_options {
            ...
            RenderOptions::Solid => {
                ...
                generate_solid_mesh(&self)
            }
            RenderOptions::Textured(texture_path) => {
                // TODO: Implement textured rendering
                todo!()
            }
            RenderOptions::LevelOfDetail(level) => {
                // TODO: Implement level of detail rendering
                todo!()
            }
            RenderOptions::Custom(custom_fn) => {
                return custom_fn();
            }
        }
    }
...
fn generate_solid_mesh(pdb: PDB) -> Mesh {
